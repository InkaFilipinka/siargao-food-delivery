...
/** POST /api/reviews/item - Submit item review */
export async function POST(request: Request) {
  try {
    const body = await request.json().catch(() => ({}));
    const { restaurantSlug, itemName, orderId, phone, rating, comment } = body;

    if (!restaurantSlug?.trim() || !itemName?.trim() || !orderId?.trim() || !phone?.trim()) {
      return NextResponse.json(
        { error: "restaurantSlug, itemName, orderId, and phone required" },
        { status: 400 }
      );
    }
    ...
    if (error) {
      console.error("Item review insert:", error);
      return NextResponse.json({ error: "Failed to save review" }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("Item review API:", err);
